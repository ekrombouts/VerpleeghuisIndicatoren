...
ps <- read_csv("../data/ps.csv")

ps <- ps %>% 
  mutate (grootte = cut(nFTE, breaks = c(1, 50, 300, Inf))) 
  
hlo = "Stichting Viva! Zorggroep"
...
hl %>% 
  ...
  geom_line(aes(y = nClient, color = "Clienten"), size = 1.1)+
...
ps %>% 
  filter (!is.na(FTEpCt)) %>% 
  ggplot (aes(jaar, FTEpCt)) +
  geom_boxplot()+
  geom_point(data = hl, mapping = aes(jaar, FTEpCt), color = "red", size = 3) +
  scale_y_log10()+
  facet_wrap(~grootte)+
  theme_bw()
```
...
hl %>% 
  select(organisatie, jaar, nFTE, niv1, niv2, niv3, niv4, niv5, niv6, beh, ovPers, ll) %>% 
  rename (behandelaren = beh,
          overig = ovPers,
          leerlingen = ll) %>% 
  filter (!is.na(nFTE)) %>% 
  ...
  mutate(niveau = ordered (niveau, levels = 
                                  c( "overig",
                                     ...
                                     "niv6",
                                     "behandelaren"))) %>% 
ggplot (aes(jaar, waarde, fill = niveau))+
  geom_col()+
...
Vergeleken met andere organisaties (jaar = 2021, groepen geven grootte van de 
organisatie weer, in aantal FTE)
```{r echo=FALSE, warning=FALSE}
hll <- hl %>% 
  select(organisatie, jaar, nFTE, niv1, niv2, niv3, niv4, niv5, niv6, beh, ovPers, ll) %>% 
  filter (jaar == 2021 & !is.na(grootte)) %>% 
  pivot_longer (cols = c(niv1:niv6),
                names_to = "niveau", 
                values_to = "waarde")  
  
ps %>% 
  select (organisatie, jaar, nFTE, niv1, niv2, niv3, niv4, niv5, niv6) %>% 
  filter (jaar == 2021 & !is.na(grootte)) %>% 
  pivot_longer (cols = c(niv1:niv6),
                names_to = "niveau", 
                values_to = "waarde") %>% 
...
ps %>% 
  select (organisatie, jaar, nFTE, beh, ovPers, ll) %>% 
  rename (behandelaren = beh,
          overig = ovPers,
          leerlingen = ll) %>% 
  filter (jaar == 2021 & !is.na(grootte)) %>% 
